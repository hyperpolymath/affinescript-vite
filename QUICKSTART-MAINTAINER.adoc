...
// Replace rsr-template-repo, road-skate, {{DEPS}} with actuals
...
| `$PREFIX/share/road-skate/`
| Data files, assets

| `$PREFIX/share/doc/road-skate/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/road-skate/config.toml`

Fallback: `$HOME/.config/road-skate/config.toml`
...
Or via OPSM: `opsm update road-skate`
...
[source,bash]
----
just install --prefix=/opt/road-skate-instance1 --config=/etc/road-skate/instance1.toml
just install --prefix=/opt/road-skate-instance2 --config=/etc/road-skate/instance2.toml
----
